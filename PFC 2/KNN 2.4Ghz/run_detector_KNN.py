...
# --- Classe de Detecção KNN (MODIFICADA) ---

class KNNJammerDetector: # Renomeado para refletir o algoritmo
    def __init__(self, config: X310Config, 
        ...
        logger.info("Iniciando o Detector de Jammers (KNN)...")
        self.config = config
        self.fft_size = fft_size
        self.running = False
        ...
        self.csv_filename = f"log_detector_knn_{timestamp_str}.csv"
        ...
        try:
            logger.info(f"Carregando modelo KNN de: {model_path}")
            # MUDANÇA AQUI: Carrega com joblib, não load_model
    ...
    def classify_spectrum(self, live_spectrum: SpectrumData) -> (int, float):
        # MUDANÇA AQUI: Formato de dados para KNN (2D)
        ...
        # Previsão KNN
        # predict() retorna a classe (0, 1 ou 2)
        prediction = self.model.predict(power_scaled)[0]
        
        # predict_proba() retorna as probabilidades ([0.1, 0.8, 0.1])
        probabilities = self.model.predict_proba(power_scaled)[0]
    ...
    def run(self):
        ...
        logger.info(f"Detector KNN iniciado. Limite de Confiança: {self.confidence_threshold}%")
        ...
        try:
            ...
            logger.info("Detector KNN parado.")

    def stop(self):
        self.running = False

def main():
    print("=== Detector de Jammers (baseado em KNN) - v1 ===")

    FFT_SIZE = 4096 
    # ATENÇÃO: Nomes dos arquivos atualizados para o padrão KNN
    MODEL_FILE = "jammer_knn_model.pkl" 
    ...
    try:
        ...
        logger.error(f"Não foi possível iniciar. Verifique se 'jammer_knn_model.pkl' existe.")
    except Exception as e:
