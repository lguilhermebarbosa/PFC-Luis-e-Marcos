...
import os
import joblib 
import csv # Para salvar o log detalhado
...
class ReactiveJammerBot:
    def __init__(self, config: X310Config, 
        ...
        logger.info("Iniciando SISTEMA REATIVO (Jammer Automático - KNN)...")
        self.config = config
        self.fft_size = fft_size
        self.running = False
        ...
        self.csv_filename = f"log_reactive_knn_{timestamp_str}.csv"
        
        self.csv_file = open(self.csv_filename, 'w', newline='')
        self.csv_writer = csv.writer(self.csv_file)
        
        self.csv_writer.writerow(['timestamp_abs', 'elapsed_time', 'class_id', 'class_name', 'confidence', 'action'])
        ...
        try:
            ...
            # O Keras (load_model) não é mais necessário e não deve ser importado se o modelo for KNN
    ...
    def classify_spectrum(self, live_spectrum: SpectrumData) -> (int, float):
        ...
        predicted_class = self.model.predict(power_scaled)[0]
        
        # predict_proba() retorna as probabilidades ([0.1, 0.8, 0.1])
        # Usamos o máximo disso como confiança
        probabilities = self.model.predict_proba(power_scaled)[0]
        
        # Mapeia a classe e a confiança
...
def main():
    ...
    FFT_SIZE = 4096 
    # MUDANÇA AQUI: Usa o modelo .pkl do KNN
    MODEL_FILE = "jammer_knn_model.pkl" 
    SCALER_FILE = "spectrum_scaler.pkl"

    ATTACK_COOLDOWN = 5.0 
    # Mantemos alta confiança, o KNN atingiu 95% no treino.
    ...
    try:
        ...
        logger.error(f"Arquivos do modelo não encontrados. Verifique se 'jammer_knn_model.pkl' existe.")
    except Exception as e:
