...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        try:
            files = [f for f in os.listdir('.') if f.startswith('log_reactive') and f.endswith('.csv')]
            if files:
                files.sort(key=os.path.getmtime, reverse=True)
                filename = files[0]
            else:
                print("Nenhum arquivo 'log_reactive*.csv' encontrado nesta pasta.")
                sys.exit(1)
        except Exception as e:
