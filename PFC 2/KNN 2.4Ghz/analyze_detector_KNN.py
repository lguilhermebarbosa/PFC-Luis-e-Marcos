...
def analyze_knn_log(csv_file):
    ...
    fig.suptitle(f'Relatório de Detecção (KNN)\nArquivo: {csv_file}', fontsize=16)
    ...
    # Scatter plot (Pontos maiores para ver os degraus do KNN)
    ax1.scatter(df['elapsed_time'], df['confidence'], c=row_colors, s=25, alpha=0.6, label='Classificação')
    
    # Marca alertas
    alerts = df[df['alert_status'] == 'ALERTA_JAMMER']
    for _, row in alerts.iterrows():
        ax1.axvline(x=row['elapsed_time'], color='red', linestyle='--', alpha=0.6)
    ...
    legend_elements = [
        ...
        Line2D([0], [0], marker='o', color='w', label='Jammer', markerfacecolor='red', markersize=8),
        Line2D([0], [0], color='red', linestyle='--', label='Alerta')
    ]
    ax1.legend(handles=legend_elements, loc='lower right')
    
    ax1.set_ylabel('Confiança (%)')
    ax1.set_title('Timeline: Classificação e Confiança (Discreta)')
    ax1.set_ylim(0, 115)
    ax1.grid(True)
    ...
    if not alert_counts.empty:
        ...
        if not counts_to_plot.empty:
            ...
            ax3.set_title('Eventos do Sistema')
            ax3.set_ylabel('Quantidade')
            ax3.tick_params(axis='x', rotation=0)
            for i, v in enumerate(counts_to_plot):
                ax3.text(i, v + 0.1, str(v), ha='center')
        else:
            ax3.text(0.5, 0.5, 'Apenas monitoramento', ha='center')
    ...
    if len(data_to_plot) > 0:
        # Boxplot mostra bem a mediana e os quartis, útil mesmo para dados discretos
        ax4.boxplot(data_to_plot, labels=labels, vert=False, patch_artist=True)
        ax4.set_title('Distribuição de Confiança (KNN)')
        ax4.set_xlabel('Confiança (%)')
    
    plt.tight_layout(rect=[0, 0.03, 1, 0.95])
    
    # Salvar e Relatar
    output_img = csv_file.replace('.csv', '_analise_knn.png')
    plt.savefig(output_img)
    print(f"Gráfico salvo em: {output_img}")
    
    print("\n" + "="*50)
    print("RELATÓRIO DE DETECÇÃO DE JAMMERS (KNN)")
    print("="*50)
    ...
    print(f"  - Alertas Confirmados: {real_alerts}")
    print(f"  - Ignorados (Baixa Confiança): {ignored_alerts}")
    print(f"  - Suprimidos (Cooldown): {cooldown_events}")
...
if __name__ == "__main__":
    if len(sys.argv) > 1:
        ...
        try:
            files = [f for f in os.listdir('.') if f.startswith('log_detector_knn') and f.endswith('.csv')]
            if files:
                files.sort(key=os.path.getmtime, reverse=True)
                filename = files[0]
            else:
                # Fallback
                filename = "log_detector_knn_20251118_201756.csv"
        except:
            sys.exit(1)
            
    analyze_knn_log(filename)